...
# FEND Multi-installer
# a one line clone-and-compile for fend:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/F-E-N-D/fend/master/scripts/multi_installer.sh" | bash
#
...
_fail() {
    ...
    _colorize red "Please check build.log and if you need help msg to 'flatearthnetworkdollar@pm.me'" && echo
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/FEND ]; then
            read -r -p "${1:-FEND directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old FEND directory" && echo
                    rm -rf "$PWD"/FEND
                    ;;
                *)
                    _fail "FEND directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir FEND
        git clone -q https://github.com/F-E-N-D/FEND FEND   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd FEND
    fi
}

_build_fend() {
    _note "Building FEND from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/F-E-N-D')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/F-E-N-D')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "FEND Multi_Installer v1.0 (pepperoni)"
_colorize white "Flat Earth Network Decentralized" && echo
...
_note "Look in 'FEND/build/src/' for the executible binaries. See 'https://github.com/F-E-N-D/FEND' for more project support."
